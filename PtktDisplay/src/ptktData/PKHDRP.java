    ...
    public synchronized PtktSum getOpnPtktByStat() throws CustomException {
       ...
       ArrayList<PtkCarton> ptkCarton = new ArrayList<>();
        ...
        String qry = "Select PKHPCK "
                ...
                + ",PKHVIA"
                + ",skuCnt"
                + ",case when PKHVIA = 'CPU' and c.cnelk = 'PAK' then "
           ...
           // get total units, total $, and sku count from carton detail subquery      
               ...
               // + "from " + dtaLib + ".ORDRPD a "
               // + "join " + dtaLib + ".ORDRPH b "
               // + " on a.orord = b.orord "
               // + "where orpck <> 0 "
               // + " and ((ordst = ' ') or (ordst = 'S' and orltd =" + today + ")) "
               // + "group by orpck, OROTY "
               // + ") b "
               // + "ON l.cdpckp = PKHPCK "
                + "join " + dtaLib + ".ORDRPH b "
                + "on b.orord = a.pkhord "
                ...
                // inner sql for pick ticket history 
//                + "left join "
//                + "(select pkspck,"
//                + "pksopcd,"
//                + "char(max(digits(pkssdt) || digits(pksstm)),14) as STGDATTM "
//                + "from " + dtaLib + ".pkhstp "
//                + "group by pkspck,pksopcd) j "
//                + "on j.pkspck = pkhpck "
//                + "and j.pksopcd = pkhopcd "
//                + "join (select CHPCK# ,count(*) CRTNCNT " 
//                + "from " + dtaLib + ".ccarph " 
//                + "group by CHPCK# ) k "
//                + " on k.CHPCK# = pkhpck"
                ;
        // inner sql to pick last person 
//                + "left join "
//                + "(SELECT pkspck "
//                + ",cnDes PICKER"
//                + ",dec(row_number() over "
//                + "(partition by pkspck order by  digits(pkssdt)|| "
//                + "digits(pksstm) desc ),5,0)  rowNum "
//                + "FROM " + dtaLib + ".pkhstp a "
//                + "join " + dtaLib + ".cntlp b "
//                + "on b.cnelt = '91' "
//                + "and b.cnelk = a.pksopr ) g "
//                + "on g.pkspck = pkhpck "
//                + " and g.rowNum = 1";
                ...
                while (rs.next()) {
                   ptkCarton.add(new PtkCarton(
                        ...
                        //    rs.getString("STGDATTM"),
                            ...
                            rs.getString("SHPNAM"),
                            rs.getInt("SKUCNT")
                           ,rs.getString("PKSOPCD")
                           ,rs.getBigDecimal("PKSSDT")
                           ,rs.getBigDecimal("PKSSTM")
                           ,rs.getBigDecimal("PKSEDT")
                           ,rs.getBigDecimal("PKSETM")
                    ));
                }

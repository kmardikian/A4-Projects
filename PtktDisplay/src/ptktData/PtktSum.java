...
public class PtktSum {
    ...
    private void crtInitSumByStat() {
        ...
        for (PtktStat ps : pstat) {
            ...
            // + ps.getStatNum());
            // }
            ptktStatMap.put(ps.getStat(), new PtktSumByStat(ps.getStat(), ps.getStatNum()));
        }
...
//
        // update sum by stat
        if (ptktStatMap.get(ptk.getStatus()) == null) {
//            System.out.println("not foud:" + ptkCrt.getStatus());
            ptktStatMap.put(ptk.getStatus(), new PtktSumByStat(ptk.getStatus(),
                    ptk.getStatusNum()));
        }
        ptktStatMap.get(ptk.getStatus()).addStat(ptk.getTotu(), ptk.getTotd(), ptk.getCrtnCnt());

...
//    private TableColumn<PtktByPkrTblRow, Integer> tcPkrAsg;
    ...
    @FXML
    private TableColumn<PtktByPkrTblRow, Integer> tcPkrPak;
    ...
    @FXML
    private TableColumn<PtktSumPrdRow, Integer> tcPrdPak;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        ObservableList<Integer> spnValues
                = FXCollections.observableArrayList(30, 60, 90, 120, 150, 180, 210, 240, 270, 300);
        SpinnerValueFactory<Integer> spnFac
                = new SpinnerValueFactory.ListSpinnerValueFactory<>(spnValues);
        spnFac.setValue(spnValues.get(2));
...
//        });
    ...
    private void bldSumPkr(ArrayList<PtktSumByPkr> sumByPkrList) {
        ...
        BigDecimal wPakU = BigDecimal.ZERO;
        BigDecimal wPakD = BigDecimal.ZERO;
...
//                        break;
                    ...
                    case AppParms.PAK_STAT_NAME:
                        wPak = wPak + statSum.getPtktCnt();
                        wPakU = wPakU.add(statSum.getTotUnt());
                        wPakD = wPakD.add(statSum.getTotDlr());
                        wPakCtn += statSum.getTotCrtn();
                        break;
        ...
        ptktByPkrTblRow.add(new PtktByPkrTblRow(stTot, wPrt, wDis, wCus, wPck,
                wPak, wCpu, wWgh, wShp, wInv, wPrtU, wPrtD, wDisU, wDisD,
                wCusU, wCusD, wPckU, wPckD, wPakU, wPakD, wCpuU, wCpuD, wWghU, wWghD,
                wShpU, wShpD, wInvU, wInvD, wPrtCtn, wDisCtn, wCusCtn,
                wPckCtn, wPakCtn, wCpuCtn, wWghCtn, wShpCtn, wInvCtn));
...
//        });
        ...
        tcPkrPak.setCellValueFactory(cellData -> cellData.getValue().getPakProperty().asObject());
        tcPkrPak.setCellFactory(tc -> {
            //ObservableList<String> cellStyle;
            TableCell<PtktByPkrTblRow, Integer> cell = new TableCell<PtktByPkrTblRow, Integer>() {
                @Override
                protected void updateItem(Integer item, boolean empty) {
                    ...
                    if ((!empty) && (item != 0)) {
                        ...
                        thisRow = getTableView().getItems().get(row);
                        tooltip.setText(
                                "Cartons: " + numFormatter.format(thisRow.getPakCtn()) + "\n"
                                + "units  : " + numFormatter.format(thisRow.getPakUnt()) + "\n"
                                + "Dollars: " + currencyFormatter.format(thisRow.getPakDlr()));
                        setTooltip(tooltip);
                        // }
                    } else {
                        this.getStyleClass().remove("selCell");
            ...
            cell.setOnMouseClicked(e -> {
                if (!cell.isEmpty()) {
                    ...
                    tblRow = cell.getTableRow();
                    PtktByPkrTblRow myItem = tblRow.getItem();
                    if (myItem.getPakProperty().getValue() != 0) {
                        dspDetail(" ", AppParms.PAK_STAT_NAME, myItem.getNamProperty().getValue());
                        //System.out.println("cell clicked " + cell.getText() + "period= " + myItem.getPrdProperty());
                    }
                }
            });
...
//        XYChart.Series<String, Number> serAsg = new XYChart.Series<>();
        ...
        XYChart.Series<String, Number> serPak = new XYChart.Series<>();
        ...
        BigDecimal tPakU = BigDecimal.ZERO;
        BigDecimal tPakD = BigDecimal.ZERO;
...
//                    sumPrd.getDisProperty(), sumPrd.getPckProperty(), sumPrd.getPakProperty(), sumPrd.getCpk(),
//                    sumPrd.getWghProperty(), sumPrd.getShpProperty(), sumPrd.getInvProperty(), sumPrd.getTotProperty()));
            ...
            tPak = tPak + ptkRow.getPak();
            tPakU = tPakU.add(ptkRow.getPakUnt());
            tPakD = tPakD.add(ptkRow.getPakDlr());
            tPakCtn += ptkRow.getPakCtn();
...
//            serAsg.getData().add(new XYChart.Data(sumPrd.getPeriod(), sumPrd.getAsg()));
            ...
            serPak.getData().add(new XYChart.Data(ptkRow.getPrd(), ptkRow.getPak()));
        ...
        pktSumTblPrdRow.add(new PtktSumPrdRow(stTot, tPrt, tDis, tCus,
                tPck, tPak, tCpu, tWgh, tShp, tInv, tPrtU, tPrtD,
                tDisU, tDisD, tCusU, tCusD, tPckU, tPckD, tPakU, tPakD,
                tCpuU, tCpuD, tWghU, tWghD, tShpU, tShpD, tInvU, tInvD,
                tPrtCtn, tDisCtn, tCusCtn, tPckCtn, tPakCtn, tCpuCtn,
        ...
        tcPrdPak.setCellValueFactory(cellData -> cellData.getValue().getPakProperty().asObject());
        tcPrdPak.setCellFactory(tc -> {
            //ObservableList<String> cellStyle;
            TableCell<PtktSumPrdRow, Integer> cell = new TableCell<PtktSumPrdRow, Integer>() {
                @Override
                protected void updateItem(Integer item, boolean empty) {
                    ...
                    if ((!empty) && (item != 0)) {
                        ...
                        Tooltip tooltip = new Tooltip();
                        tooltip.setText(
                                "Cartons: " + numFormatter.format(curRow.getPakCtn()) + "\n"
                                + "Units  : " + numFormatter.format(curRow.getPakUnt()) + "\n"
                                + "Dollars: " + currencyFormatter.format(curRow.getPakDlr()));
                        setTooltip(tooltip);
                    } else {
                        this.getStyleClass().remove("selCell");
                    }
            ...
            cell.setOnMouseClicked(e -> {
                if (!cell.isEmpty()) {
                    ...
                    PtktSumPrdRow myItem = tblRow.getItem();
                    if (myItem.getPakProperty().getValue() != 0) {
                        dspDetail(myItem.getPrdProperty().getValue(), "PAK", AppParms.ALL);
                    }
                }
            });
            return cell;
        ...
        bChartPtktPrd.getData().addAll(serPrt, serDis, serCus, serPck, serPak, serCpk, serWgh, serShp, serInv);
...
//        serAsg.setName("Asg");
        ...
        serPak.setName(AppParms.PAK_STAT_NAME);
...
//            Tooltip.install(dtDis.getNode(), tooltip);
        ...
        for (final XYChart.Data<String, Number> dtPak : serPak.getData()) {
//            Tooltip tooltip = new Tooltip();
...
//            Tooltip.install(dtPak.getNode(), tooltip);
            dtPak.getNode().setOnMouseClicked((MouseEvent e) -> {
                dspDetail(dtPak.getXValue(), "PAK", AppParms.ALL);

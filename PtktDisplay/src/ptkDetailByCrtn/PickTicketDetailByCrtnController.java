...
//        loadDtlDsp();
    ...
    private void exportPtktData(File selFile) {
        ...
        for (PickTicketDetailByCartonRow tr : tblRows) {
            ...
            wMin = Integer.valueOf(sTime.substring(2, 4));
            wSec = Integer.valueOf(sTime.substring(4, 6));
            wStatDt.set(wYear, wMon, wDa, wHr, wMin, wSec);
            ...
            wMin = Integer.valueOf(sTime.substring(2, 4));
            wSec = Integer.valueOf(sTime.substring(4, 6));
            wCmpDt.set(wYear, wMon, wDa, wHr, wMin, wSec);
            enTime = sTime;
            ...
            for (colIx = 0; colIx < tblCol.size(); colIx++) {
                ...
                switch (col.getText()) {
                    ...
                    case COL_PRT_TM:
                        ...
                        break;
                    case COL_ORD_STRDT:
                        cell.setCellValue(dfDat8.format(tr.getOrStrDt().getValue()));
                        cell.setCellStyle(datCellStyle);
                        break;
                    case COL_ORD_CMPDT:
                        cell.setCellValue(dfDat8.format(tr.getOrCmpDt().getValue()));
                        cell.setCellStyle(datCellStyle);
                        break;
                }
            }
